...
define("NLPCAPTCHA__VALIDATE_TIMEOUT", '0');
...
function Nlp_Validate($nlpIdentifier,$nlpAnswer,$validatekey){
		...
		$response =  Nlp_curl_post(NLPCAPTCHA_VALIDATE_URL,$arr_params);
                }
                else
                {
                  
                $response=Nlp_post_request(NLPCAPTCHA_VALIDATE_URL, $arr_params);  
                }
			//print_r($response);
 ...
 * @return string 
 */ 
function Nlp_curl_post($url, array $post = NULL, array $options = array()) 
{ 
    $defaults = array( 
        CURLOPT_POST => 1, 
        CURLOPT_HEADER => 0, 
   ...
   if(NLP_VALIDATE_TIMEOUT>0)
       {
	$defaults[CURLOPT_TIMEOUT]=NLPCAPTCHA_VALIDATE_TIMEOUT;
	}
    $ch = curl_init(); 
...
function Nlp_post_request($url, $data, $referer='') {
...
function Nlp_get_signup_url ($domain = null, $appname = null) {
