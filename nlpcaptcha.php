...
require_once( plugin_dir_path(__FILE__) . 'wp-plugin.php');
    ...
    class Nlp_NLPCaptcha extends Nlp_WPPlugin {
        // member variables
        private $saved_error;
        
        // php 4 constructor
        function Nlp_NLPCaptcha($options_name) {
            $args = func_get_args();
        ...
        function __construct($options_name) {
            ...
            $this->nlp_register_default_options();
            
            // require the nlpcaptcha library
            //$this->require_library();
            
            // register the hooks
            $this->nlp_register_actions();
            $this->nlp_register_filters();
        }
        
        function nlp_register_actions() {
            // load the plugin's textdomain for localization
            add_action('init', array(&$this, 'nlp_load_textdomain'));

            // styling
            add_action('wp_head', array(&$this, 'nlp_register_stylesheets')); // make unnecessary: instead, inform of classes for styling
            add_action('admin_head', array(&$this, 'nlp_register_stylesheets')); // make unnecessary: shouldn't require styling in the options page
            
            if ($this->options['show_in_registration'])
                add_action('login_head', array(&$this, 'nlp_registration_style')); // make unnecessary: instead use jQuery and add to the footer?

            // options
            register_activation_hook(plugin_dir_path( __FILE__ ).'wp-nlpcaptcha.php', array(&$this, 'nlp_register_default_options')); // this way it only happens once, when the plugin is activated
            add_action('admin_init', array(&$this, 'nlp_register_settings_group'));

            // only register the hooks if the user wants nlpcaptcha on the registration page
            if ($this->options['show_in_registration']) {
                // nlpcaptcha form display
                if ($this->nlp_is_multi_blog())
                    add_action('signup_extra_fields', array(&$this, 'nlp_show_nlpcaptcha_in_registration'));
                else
                    add_action('register_form', array(&$this, 'nlp_show_nlpcaptcha_in_registration'));
            }

            // only register the hooks if the user wants nlpcaptcha on the comments page
            if ($this->options['show_in_comments']) {
                add_action('comment_form', array(&$this, 'nlp_show_nlpcaptcha_in_comments'));

                // nlpcaptcha comment processing (look into doing all of this with AJAX, optionally)
                add_action('wp_head', array(&$this, 'nlp_saved_comment'), 0);
                add_action('preprocess_comment', array(&$this, 'nlp_check_comment'), 0);
                add_action('comment_post_redirect', array(&$this, 'nlp_relative_redirect'), 0, 2);
            }

            // administration (menus, pages, notifications, etc.)
            add_filter("plugin_action_links", array(&$this, 'nlp_show_settings_link'), 10, 2);

            add_action('admin_menu', array(&$this, 'nlp_add_settings_page'));
            
            // admin notices
            add_action('admin_notices', array(&$this, 'nlp_missing_keys_notice'));
        }
        
        function nlp_register_filters() {
            // only register the hooks if the user wants nlpcaptcha on the registration page
            if ($this->options['show_in_registration']) {
                // nlpcaptcha validation
                if ($this->nlp_is_multi_blog())
                    add_filter('wpmu_validate_user_signup', array(&$this, 'nlp_validate_nlpcaptcha_response_wpmu'));
                else
                    add_filter('registration_errors', array(&$this, 'nlp_validate_nlpcaptcha_response'));
            }
        }
        
        function nlp_load_textdomain() {
            load_plugin_textdomain('nlpcaptcha', false, 'languages');
        }
        
        // set the default options
        function nlp_register_default_options() {
            if ($this->options)
               return;
           
            $option_defaults = array();
           
            $old_options = get_option("nlpcaptcha");
        ...
        // require the nlpcaptcha library
        function nlp_require_library() {
            require_once(plugin_dir_path( __FILE__ ).'nlpcaptchalib.php');
        }
        
        // register the settings
        function nlp_register_settings_group() {
            register_setting("nlpcaptcha_options_group", 'nlpcaptcha_options', array(&$this, 'nlp_validate_options'));
        }
        
        // todo: make unnecessary
        function nlp_register_stylesheets() {
            $path = plugin_dir_path( __FILE__ ).'nlpcaptcha.css';
        ...
        function nlp_registration_style() {
...
REGISTRATION;
        ...
        function nlp_missing_keys_notice() {
            if ($this->nlp_nlpcaptcha_enabled() && $this->nlp_keys_missing()) {
                $this->nlp_create_error_notice('You enabled NLPCaptcha, but some of the NLPCaptcha API Keys seem to be missing.');
            }
        }
        
        function nlp_validate_dropdown($array, $key, $value) {
        ...
        function nlp_validate_options($input) {
            ...
            $validated['minimum_bypass_level'] = $this->nlp_validate_dropdown($capabilities, 'minimum_bypass_level', $input['minimum_bypass_level']);
        ...
        // display nlpcaptcha
        function nlp_show_nlpcaptcha_in_registration($errors) {
...
COMMENT_FORM;
            ...
            if ($this->nlp_is_multi_blog()) {
                $error = $errors->nlp_get_error_message('captcha');
                echo '<label for="verification">Verification:</label>';
                echo ($error ? '<p class="error">'.$error.'</p>' : '');
                echo  $this->nlp_get_nlpcaptcha_html($escaped_error, $use_ssl);
            }
        ...
        function nlp_validate_nlpcaptcha_response($errors) {
        ...
        function nlp_validate_nlpcaptcha_response_wpmu($result) {
        ...
        function nlp_get_nlpcaptcha_html($nlpcaptcha_error, $use_ssl=false) {
            return NLP_Captcha($this->options['publisherkey']);
        }
        
        function nlp_show_nlpcaptcha_in_comments() {
            global $user_ID;
...
COMMENT_FORM;
                ...
                echo $this->nlp_get_nlpcaptcha_html(isset($escaped_error) ? $escaped_error : null, $use_ssl) . $comment_string;
           }
        }
        
        // this is what does the submit-button re-ordering
        function nlp_save_comment_script() {
            $javascript = <<<JS
                <script type="text/javascript">
...
JS;
        ...
        function nlp_show_captcha_for_comment() {
            global $user_ID;
            return true;
        }
        
        function nlp_check_comment($comment_data) {
            global $user_ID;
            ...
            if ($this->nlp_show_captcha_for_comment()) {
        ...
        function nlp_relative_redirect($location, $comment) {
            if ($this->saved_error != '') {
                ...
                $location = substr($location, 0, strpos($location, '#')) .
                    ...
                    '&rchash=' . $this->nlp_hash_comment($comment->comment_ID) .
                    '#commentform';
            }
        ...
        function nlp_saved_comment() {
            ...
            if ($comment_hash == $this->nlp_hash_comment($comment_id)) {
        ...
        function nlp_blog_domain() {
            $uri = parse_url(get_option('siteurl'));
            return $uri['host'];
        }
        
        // add a settings link to the plugin in the plugin list
        function nlp_show_settings_link($links, $file) {
            if ($file == plugin_basename(plugin_dir_path( __FILE__ ). 'wp-nlpcaptcha.php')) {
        ...
        function nlp_add_settings_page() {
            // add the options page
            if ($this->environment == Nlp_Environment::WordPressMU && $this->nlp_is_authority())
                add_submenu_page('wpmu-admin.php', 'NLPCaptcha', 'NLPCaptcha', 'manage_options', __FILE__, array(&$this, 'nlp_show_settings_page'));

           
            add_options_page('NLPCaptcha', 'NLPCaptcha', 'manage_options', __FILE__, array(&$this, 'nlp_show_settings_page'));
        }
        
        // store the xhtml in a separate file and use include on it
        function nlp_show_settings_page() {
            include("settings.php");
        }
        
        function nlp_build_dropdown($name, $keyvalue, $checked_value) {
        ...
        function nlp_capabilities_dropdown() {
            ...
            $this->nlp_build_dropdown('nlpcaptcha_options[minimum_bypass_level]', $capabilities, $this->options['minimum_bypass_level']);
        }
